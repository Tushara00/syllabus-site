...
let sessionsRaw = raw.split(/\n?---\n?/);
	...
	sessionsRaw.map((sessionRaw, sessionIndex) => {
		console.log(`📌 Processing Session ${sessionIndex}:`, sessionRaw);
		...
		let cleanedSessionRaw = sessionRaw.trim().replace(/^\s+/, '');
		let sessionContent = cleanedSessionRaw.split(/\r?\n{2,}/);
		console.log(`📌 Session stringyfy ${sessionIndex}:`, JSON.stringify(sessionRaw));
		console.log(`📖 Session ${sessionIndex} Content:`, sessionContent);
		console.log("Session Index:", sessionIndex);
console.log("Session Content:", sessionContent);
if (sessionContent.length === 0) {
	console.warn(`⚠️ Session ${sessionIndex} has no content!`);
	return;
}
		let session = {}
		...
		sessionContent.map((content, contentIndex) => {
			...
			console.log(`📝 Creating Item ${contentIndex} for Session ${sessionIndex}`);
			// CREATE HTML ELEMENT 
